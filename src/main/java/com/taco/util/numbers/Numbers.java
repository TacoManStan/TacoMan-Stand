...
public class Numbers
{
    ...
    public static boolean isNumber(String input, String... args)
    {
        ...
        if (input == null)
            if (com.taco.util.numbers.Filter.THROW_NPE.matches(argsList))
        ...
        try
        {
            ...
            if (Double.isNaN(parsedInput) && !com.taco.util.numbers.Filter.ALLOW_NaN.matches(argsList))
                return false;
            else if (Double.isInfinite(parsedInput) && !com.taco.util.numbers.Filter.ALLOW_INFINITY.matches(argsList))
                return false;
            else if (decimals(parsedInput) != 0 && com.taco.util.numbers.Filter.INT_ONLY.matches(argsList))
                return false;
        ...
        {
            if (com.taco.util.numbers.Filter.THROW_NFE.matches(argsList))
                throw e;
            return false;
        }
    }
    ...
    public static boolean isInteger(String input, boolean throwNPE, boolean throwNFE)
    {
        ...
        filters.add(com.taco.util.numbers.Filter.INT_ONLY.key());
        if (throwNPE)
            filters.add(com.taco.util.numbers.Filter.THROW_NPE.key());
        if (throwNFE)
