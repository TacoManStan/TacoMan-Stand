package com.taco.tacository.util.tools.fx_tools;

import com.taco.tacository.util.tools.Enu;
import com.taco.tacository.util.tools.Stuff;
import com.taco.tacository.util.tools.Str;
import com.taco.tacository.util.tools.TB;
import com.taco.tacository.ui.jfx.dialog.DialogCallback;
import com.taco.tacository.ui.jfx.dialog.DialogController;
...
public class DialogsFX {
    ...
    public static <T extends Enum> T showChooseEnumDialog(String title, String message, FX.AutoCompleteMode autoCompleteMode, T defaultOption, String promptText, boolean selectFirst) {
        if (promptText != null && promptText.isEmpty()) promptText = "Start typing " + Str.classToString(defaultOption.getClass());
        return showChooseOptionDialog(title, message, autoCompleteMode, defaultOption, selectFirst, promptText, (T[]) Enu.list(defaultOption));
    }
    ...
    public static <T> T showDialog(String title, String message, Node content, Node contentToFocus, double spacing, ButtonType[] buttonTypes, SizeType sizeType, DialogCallback<T> callback) {
        return FX.callFX(() -> {
            ...
            // Apply css to dialog
            dialog_pane.getStylesheets().add(Stuff.getResourceURL("css/main.css").toExternalForm()); dialog_pane.getStyleClass().add("dialogFX");
            ...
            new Thread(() -> {
                ...
                while (!dialog.isShowing()) TB.sleep(5);
                // Focus content if not a ComboBox, ChoiceBox, or ListView
                FX.runFX(() -> {
                    if (final_content_to_focus != null && !(TB.instanceOf(final_content_to_focus, ComboBox.class, ChoiceBox.class, ListView.class))) final_content_to_focus.requestFocus();
                }, false);
            }).start();
