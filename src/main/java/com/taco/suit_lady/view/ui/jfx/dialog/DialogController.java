		...
		extends Controller
{
	 ...
	 * <p>
	 * Returns {@link FXDialogTools#DONE} if the specified array is null or empty.
	 *
	...
	private ButtonType[] setButtonTypes(ButtonType... buttonTypes) {
		if (buttonTypes != null && buttonTypes.length > 0)
			if (!ArrayTools.containsNull(buttonTypes))
				return buttonTypes;
			else
				throw new NullPointerException("No ButtonType can be null.");
		return FXDialogTools.DONE_CANCEL;
	}
	...
	protected R show(boolean launch) {
		value = FXDialogTools.showControllableDialog(title, message, spacing, buttonTypes, launch, this);
		onClose();
		return value;
	}
	 ...
	 * {@link FXDialogTools#showControllableDialog(String, String, double, ButtonType[], boolean, DialogController) dialog}.
	 *
