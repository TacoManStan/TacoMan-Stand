...
public class Sidebar {
	...
	public Sidebar(StackPane childButtonPane, StackPane contentPane, ImagePane backImagePane) {
		...
		this.backImageButton = new ImageButton(backImagePane, "back_arrow", () -> FXTools.get().runFX(this::back, false), false, true, ImageButton.SMALL);
		...
		this.selectedNodeGroupProperty.addListener((observable, oldNodeGroup, newNodeGroup) -> {
				if (!Objects.equals(oldNodeGroup, newNodeGroup)) { //If the menu base is already selected, return silently and do nothing.
					if (oldNodeGroup != null) {
						VBox oldButtonBox = oldNodeGroup.getButtonBox();
						if (oldButtonBox != null)
							childButtonPane.getChildren().remove(oldButtonBox);
					}
					if (newNodeGroup != null) {
						VBox buttonBox = newNodeGroup.getButtonBox();

						childButtonPane.getChildren().add(buttonBox);
						contentPane.getChildren().clear();
						contentPane.getChildren().add(newNodeGroup.getContent());
					}
				}
		});
	}
