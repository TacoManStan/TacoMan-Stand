...
public class DialogsFX {
     ...
     */
    public static final ButtonType[] YES_NO_CANCEL = {new ButtonType("Yes", ButtonBar.ButtonData.YES), new ButtonType("No", ButtonBar.ButtonData.NO), new ButtonType("Cancel", ButtonBar.ButtonData.CANCEL_CLOSE)};
    ...
    public static void showMessageDialog(String title, String message) {
        ToolsFX.runFX(() -> showDialog(title, message, null, null, DEFAULT_SPACING, OK, SizeType.RESIZEABLE, null), true);
    }
    ...
    public static boolean showConfirmDialog(String title, String message) {
        Boolean bool = showDialog(title, message, null, null, DEFAULT_SPACING, YES_NO, SizeType.RESIZEABLE, new DialogCallback<>(false, (param) -> true)); return bool != null ? bool : false;
    }
    ...
    public static ButtonBar.ButtonData showYesNoCancelDialog(String title, String message) {
        return showDialog(title, message, null, null, DEFAULT_SPACING, YES_NO_CANCEL, SizeType.RESIZEABLE, new DialogCallback<>(param -> param != null ? param.getButtonData() : null));
    }
    ...
    public static int showIntDialog(String title, String message, int... initialValue) {
        int initialVal = initialValue != null && initialValue.length != 0 ? initialValue[0] : 0; TextField textField = new TextField(); ToolsFX.numberTextField(textField, false); textField.setText("" + initialVal);
        Integer value = showDialog(title, message, textField, textField, DEFAULT_SPACING, DONE_CANCEL, SizeType.RESIZEABLE, new DialogCallback<>(NULL_NUMBER, param -> ToolsFX.getIntValue(textField))); return value != null ? value : NULL_NUMBER;
    }
    ...
    public static long showLongDialog(String title, String message, long... initialValue) {
        long initialVal = initialValue != null && initialValue.length != 0 ? initialValue[0] : 0; TextField textField = new TextField(); ToolsFX.numberTextField(textField, false); textField.setText("" + initialVal);
        Long value = showDialog(title, message, textField, textField, DEFAULT_SPACING, DONE_CANCEL, SizeType.RESIZEABLE, new DialogCallback<>((long) NULL_NUMBER, param -> ToolsFX.getLongValue(textField))); return value != null ? value : NULL_NUMBER;
    }
    ...
    public static double showDoubleDialog(String title, String message, double... initialValue) {
        double initialVal = initialValue != null && initialValue.length != 0 ? initialValue[0] : 0.0; TextField textField = new TextField(); ToolsFX.numberTextField(textField, true); textField.setText("" + initialVal);
        Double value = showDialog(title, message, textField, textField, DEFAULT_SPACING, DONE_CANCEL, SizeType.RESIZEABLE, new DialogCallback<>((double) NULL_NUMBER, param -> ToolsFX.getValue(textField))); return value != null ? value : NULL_NUMBER;
    }
    ...
    public static String showStringDialog(String title, String message, boolean allowEmpty, String... initialValue) {
        String value; do {
            String initialVal = initialValue != null && initialValue.length != 0 ? initialValue[0] : ""; TextField textField = new TextField(); textField.setText(initialVal);
            ...
            if (value != null && value.isEmpty() && !allowEmpty) showMessageDialog("You must enter at least 1 character.", null);
        } while (value != null && value.isEmpty() && !allowEmpty); return value;
    }
    ...
    public static String showTextDialog(String title, String message, String... initialValue) {
        String initialVal = initialValue != null && initialValue.length != 0 ? initialValue[0] : ""; TextArea area = new TextArea(); area.setText(initialVal);
     ...
     * @param title            The title of the {@link Dialog}.
     * @param message          The message to display for the user. Null to use the title as the message.
     * @param autoCompleteMode The {@link ToolsFX.AutoCompleteMode}, or null if the {@link ComboBox} is not an auto-complete box.
     ...
     */
    @SafeVarargs public static <T> T showChooseOptionDialog(String title, String message, ToolsFX.AutoCompleteMode autoCompleteMode, T defaultOption, boolean selectFirst, String promptText, T... options) {
        ComboBox<T> comboBox = new ComboBox<>(); ToolsFX.autoCompleteComboBox(comboBox, autoCompleteMode, promptText); comboBox.getItems().addAll(options); if (autoCompleteMode == null) if (selectFirst) comboBox.getSelectionModel().selectFirst();
        else comboBox.getSelectionModel().select(defaultOption);
        return showDialog(title, message, comboBox, autoCompleteMode != null ? comboBox : null, DEFAULT_SPACING, DONE_CANCEL, SizeType.RESIZEABLE, new DialogCallback<>(defaultOption, param -> ToolsFX.getSelectedValue(comboBox)));
    }
     ...
     * @return The value that the user chose from the {@link ComboBox}, or the default value if the {@link Dialog} was cancelled or closed.
     *
     * @see #showChooseOptionDialog(String, String, ToolsFX.AutoCompleteMode, Object, boolean, String, Object[])
     */
    ...
    @SafeVarargs public static <T> T[] showChooseOptionsDialog(String title, String message, T[] empty, T... options) {
        ListView<T> listView = new ListView<>(); ToolsFX.applyCellFactory(listView); listView.getSelectionModel().setSelectionMode(SelectionMode.MULTIPLE); listView.setMinSize(0, 0); listView.setPrefSize(200, 250); listView.getItems().addAll(options);
     ...
     */
    public static <R> R showControllableDialog(String title, String message, double spacing, ButtonType[] buttonTypes, boolean launch, @NotNull DialogController<R> controller) {
        // CHANGE-HERE
        //        if (launch)
        //            controller.launch();
    ...
    public static <T> T showDialog(String title, String message, Node content, Node contentToFocus, double spacing, ButtonType[] buttonTypes, SizeType sizeType, DialogCallback<T> callback) {
        return ToolsFX.callFX(() -> {
            ...
            if (callback != null) {
                callback.init(); dialog = callback.getDialog(); dialog.setResultConverter(callback.getCallback());
            } else dialog = new Dialog<>();
            
            dialog_pane = dialog.getDialogPane();
            
            // Apply css to dialog
            dialog_pane.getStylesheets().add(ResourcesSL.getResourceURL("css/main.css").toExternalForm()); dialog_pane.getStyleClass().add("dialogFX");
            ...
            final double final_spacing = spacing == -1 ? DEFAULT_SPACING : spacing; if (final_spacing > 0 && content != null) {
                container = new AnchorPane(content); AnchorPane.setLeftAnchor(content, final_spacing); AnchorPane.setRightAnchor(content, final_spacing); AnchorPane.setTopAnchor(content, final_spacing);
            } else container = content;
            
            // Set the dialog's title (if applicable)
            if (title != null) dialog.setTitle(title);
            
            // Set the dialog's message (if applicable)
            if (message != null && !message.isEmpty()) parent_container.setTop(new Label(message));
            
            if (container != null) parent_container.setCenter(container); if (container instanceof Region && ((Region) container).getPrefWidth() < 300) parent_container.setPrefWidth(300);
            // Set the dialog's content
            dialog.getDialogPane().setContent(parent_container);
            
            // Add the option buttons to the dialog
            for (ButtonType buttonType: buttonTypes)
                dialog.getDialogPane().getButtonTypes().add(buttonType); dialog.setResizable(true);
            
            // Add dialog to the dialogs list
            synchronized (dialogs_lock) {
                if (canAdd) dialogs.add(dialog);
            }
            // Set the dialog to be removed from the dialogs list when closed
            dialog.setOnCloseRequest((DialogEvent event) -> {
                synchronized (dialogs_lock) {
                    if (canAdd) dialogs.remove(dialog);
                }
            });
            
            // Set the dialog's resize type
            if (SizeType.NON_RESIZEABLE == sizeType) dialog.setResizable(false);
            else if (SizeType.RESIZEABLE == sizeType) dialog.setResizable(true);
            
            // TODO [S]: Use Executor?
            new Thread(() -> {
                // TODO [S]: Make this more efficient
                // Wait for dialog to appear
                while (!dialog.isShowing()) ToolsSL.sleep(5);
                // Focus content if not a ComboBox, ChoiceBox, or ListView
                ToolsFX.runFX(() -> {
                    if (final_content_to_focus != null && !(ToolsSL.instanceOf(final_content_to_focus, ComboBox.class, ChoiceBox.class, ListView.class))) final_content_to_focus.requestFocus();
                }, false);
            }).start();
            
            // Set dialog to return value of specified Callback (if applicable)
            if (callback != null) {
                Optional<T> oValue = dialog.showAndWait(); if (oValue != null && oValue.isPresent()) return oValue.get();
            } else dialog.showAndWait(); return null;
        });
    }
     ...
     */
    @Contract(pure = true)
    public static ButtonBar.ButtonData @NotNull [] buttonData(ButtonType @NotNull [] buttonTypes) {
        ButtonBar.ButtonData[] buttonData = new ButtonBar.ButtonData[buttonTypes.length]; for (int i = 0; i < buttonTypes.length; i++)
            buttonData[i] = buttonTypes[i].getButtonData(); return buttonData;
    }
    ...
    public static void closeAll() {
        synchronized (dialogs_lock) {
            canAdd = false; if (dialogs != null) {
                dialogs.forEach(Dialog::close); dialogs.clear(); dialogs = null;
            } canAdd = true;
        }
    }
