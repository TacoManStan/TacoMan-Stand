...
public class Node {
    ...
    private boolean foundFromStart;
    ...
    public final Node addNeighbor(Node neighbor) {
        DebugTimer t = new DebugTimer("Add Neighbor", 500);
    ...
    private void addCacheNeighbor(int xOffset, int yOffset) {
        DebugTimer t = new DebugTimer("Add Cache Neighbor", 500);
//        ArrayList<TeleportObject> tOS = AStarCache.getTeleports(toRSTile(null));
//        if (tOS != null) {
//            for (TeleportObject tO : tOS) {
//                RSTile tempTile = tO.getTarget();
//                tempTile = tempTile != null ? tempTile.translate(xOffset, yOffset) : null;
//                if (tempTile != null) {
//                    addNeighbor(host.createNode(tempTile));
//                }
//            }
//        }
        RSTile tempTile = toRSTile(null);
        tempTile = tempTile != null ? tempTile.translate(xOffset, yOffset) : null;
        t.print();
        addNeighbor(host.createNode(tempTile));
    }
    ...
    public final boolean isFoundFromStart() {
...
//        }
    ...
    public RSTile toRSTile(RSTile tile) {
        RSTile baseTile = new RSTile(x, y, plane);
//        if (tile != null && cacheData.isTeleport()) {
//            ArrayList<TeleportObject> tOs = AStarCache.getTeleports(baseTile);
//            if (tOs != null) {
//                TeleportObject cTO = null;
//                for (TeleportObject tO : tOs) {
//                    if (Entities.distanceTo(tile, tO.getTarget()) < Entities.distanceTo(tile, baseTile)
//                            && (cTO == null || Entities.distanceTo(tile, tO.getTarget()) < Entities.distanceTo(tile, cTO.getTarget()))) {
//                        cTO = tO;
//                    }
//                }
//                if (cTO != null) {
//                    return cTO.getTarget();
//                }
//            }
//        }
        return baseTile;
    ...
    public final boolean canWalkTo(Node source) {
        return (!isStandardBlocked() || containsObstacle()) && subCanWalkTo(source);
    }
    ...
    protected boolean containsTeleport() {
        return cacheData != null && cacheData.containsTeleport();
    }
    
    protected boolean containsDoor() {
        return cacheData != null && cacheData.containsDoor();
    }
    
    protected boolean containsBlockade() {
        return cacheData != null && cacheData.containsBlockade();
    }
...
//        if (cacheData.isTeleport() != source.getCacheData().isTeleport()) {
//            return true;
//        } else {
//            return AStarCache.canWalkTo(cacheData, source.getCacheData());
//        }
//        return host.isIgnoringHandleableObstacles() ? AStarCache.canWalkTo(cacheData, source.getCacheData()) : Map07.canWalkTo(toRSTile(null), source.toRSTile(null));
        return AStarCache.canWalkTo(cacheData, source.getCacheData());
    }
